...
function exportButton(){return '<button class="button secondary export" type="button">Download result ↓</button>';}
function installExport(){
  document.querySelector('.export')?.addEventListener('click',()=>{
    const blob=new Blob([JSON.stringify(lastExport,null,2)],{type:'application/json'});
    const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${lastExport.demo}-result.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
  });
}
